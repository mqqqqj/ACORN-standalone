...
ACORNStats ACORN::parallel_hybrid_search(
        DistanceComputer& qdis,
        int k,
        idx_t* I,
        float* D,
    ...
    if (entry_point == -1) return stats;
    if (num_threads <= 1) {
        return hybrid_search(qdis, k, I, D, vt, filter_map, params);
    }
...
#pragma omp parallel num_threads(num_threads)
        {
            ...
            for (int i = start; i < end; i++) {
                auto& p = batch[i];
                if (!filter_map[p.second]) continue;
                local_queue.push(p.second, p.first);
                if (local_nres < k) {
                    maxheap_push(++local_nres, local_D.data(), local_I.data(), p.first, p.second);
                } else if (p.first < local_D[0]) {
...
#pragma omp critical
        ...
        // Collect unexpanded, filter, keep top-efs
        std::vector<std::pair<float, storage_idx_t>> all_unexpanded;
        for (int t = 0; t < num_threads; t++) {
            for (auto& p : thread_unexpanded[t]) {

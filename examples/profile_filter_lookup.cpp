...
int main(int argc, char **argv)
{
    ...
    if (!index_file || !query_file || !out_file || k <= 0 || threads <= 0 ||
    ...
    std::fprintf(out, "selectivity\ttrue_s\tfilter_cost\tstrategy\tefs\tthreads\tk\tnq\tavg_ms\ttime_ms\trecall\tndc\tok\tempty\twrong\tstatus\n");
    ...
    for (const auto &cfg : configs)
    {
        ...
        std::string gt_path = train_dir + "/laion10m_train_gt_binary_" + cfg.gt_suffix + ".ibin";
        ...
        for (int cost : costs)
        {
            ...
            for (const std::string &strategy : strategies)
            {
                ...
                {
                    std::fprintf(out, "%s\t%.6f\t%d\t%s\t0\t%d\t%d\t%d\tnan\tnan\tnan\t0\t0\t0\t0\tskipped_pre_high_selectivity\n",
                                 cfg.sel, cfg.true_s, cost, strategy.c_str(), threads, k, nq);
                    std::fflush(out);
                    continue;
                }
                if (is_pre && !contains_int(pre_costs, cost))
                {
                    std::fprintf(out, "%s\t%.6f\t%d\t%s\t0\t%d\t%d\t%d\tnan\tnan\tnan\t0\t0\t0\t0\tskipped_pre_cost\n",
                                 cfg.sel, cfg.true_s, cost, strategy.c_str(), threads, k, nq);
                    std::fflush(out);
                    continue;
                }
                ...
                for (int efs : *efs_values)
                {
                    ...
                    RunResult r = run_method(strategy, index, queries, qd, nq, k, efs,
                                             threads, Helec, metric, wf, qlabel,
                                             base_labels, gt_ids, gt_k);
                    std::fprintf(out, "%s\t%.6f\t%d\t%s\t%d\t%d\t%d\t%d\t%.3f\t%.1f\t%.4f\t%zu\t%d\t%d\t%d\tok\n",
                                 cfg.sel, cfg.true_s, cost, strategy.c_str(), efs,
                                 threads, k, nq, r.avg_ms, r.total_ms, r.recall, r.ndc,
                    ...
                    if (!is_pre && max_avg_ms > 0.0 && r.avg_ms > max_avg_ms)
                    {
                        for (int skipped_efs : *efs_values)
                        {
                            if (skipped_efs <= efs)
                                continue;
                            std::fprintf(out, "%s\t%.6f\t%d\t%s\t%d\t%d\t%d\t%d\tnan\tnan\tnan\t0\t0\t0\t0\tskipped_max_avg_ms\n",
                                         cfg.sel, cfg.true_s, cost, strategy.c_str(), skipped_efs,
                                         threads, k, nq);
                        }
                        std::fflush(out);
                        break;

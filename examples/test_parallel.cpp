...
int main(int argc, char* argv[]) {
    ...
    int k = 100, ef = 200, num_threads = 2, num_queries = 1000, efs = 200;
    ...
    // Pre-index label-to-IDs for fast filter-map generation
    ...
    {
        std::vector<int> dummy_labels(search_k * 100);
        ...
        for (int i = 0; i < 1; i++)
            index.search(1, queries.data() + i * qd, search_k,
                         dummy_dist.data(), dummy_labels.data(),
                         wf.data(), &params);
    }
    printf("  Done.\n");
    ...
    for (int i = 0; i < num_queries; i++) {
        ...
        for (int j = 0; j < search_k; j++) {
            int id = ser_labels[i * search_k + j];
            if (id >= 0 && base_labels[id] == ql) pass++;
        }
    }
    printf("  Filter compliance: %d/%d (%.1f%%)\n", pass, total, 100.0 * pass / total);

    // --- Parallel filtered search ---
    printf("\n--- Parallel Filtered Search (threads=%d, efs=%d) ---\n", num_threads, efs);
    fflush(stdout);
    std::vector<int> par_labels(search_k * num_queries);
    std::vector<float> par_dist(search_k * num_queries);

    std::vector<char> all_fm(num_queries * index.ntotal, 0);
    for (int i = 0; i < num_queries; i++) {
        int ql = query_labels[i];
        char* fm = all_fm.data() + i * index.ntotal;
        for (int id : label_to_ids[ql]) fm[id] = 1;
    }

    fprintf(stderr, "  parallel starting\n");
    t0 = get_ms();
    index.parallelSearch(num_queries, queries.data(), search_k,
                         par_dist.data(), par_labels.data(),
                         all_fm.data(), num_threads, efs, &params);
    fprintf(stderr, "  parallel done\n");

...
int main(int argc, char *argv[])
{
    ...
    // const char *gt_file = "../data/sift1m_gt_top100.ibin";
    ...
    for (int efi = 0; efi < 3; efi++)
    {
        ...
        int hist[11] = {0}; // 0-0.1, 0.1-0.2, ..., 0.9-1.0
        for (int i = 0; i < nq_all; i++)
        {
            int q_hits = 0;
            std::vector<int> gt_set(gt_ids.begin() + i * search_k, gt_ids.begin() + (i + 1) * search_k);
            std::sort(gt_set.begin(), gt_set.end());
            for (int j = 0; j < search_k; j++)
            {
                int rid = labels[i * search_k + j];
                if (rid >= 0 && std::binary_search(gt_set.begin(), gt_set.end(), rid))
                    q_hits++;
            }
            double q_recall = (double)q_hits / search_k;
            int bucket = (int)(q_recall * 10);
            if (bucket < 0)
                bucket = 0;
            if (bucket > 10)
                bucket = 10;
        ...
        // printf("  Per-query Recall distribution:\n");
        // for (int b = 0; b < 10; b++)
        //     printf("    %.1f-%.1f: %d\n", b*0.1, (b+1)*0.1, hist[b]);
        // printf("    0.9-1.0: %d\n", hist[10]);
        fflush(stdout);
    }
